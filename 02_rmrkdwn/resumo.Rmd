...
caminho_export_rotas <- here("03_export")
...
dados_trajetos %>%
  as_tibble() %>%
  group_by(ID_arquivo) %>%
  arrange(ID_arquivo, ID_ponto) %>%
  select(Data, ID_arquivo, lng, lat) %>%
  sfheaders::sf_linestring(
    x = "lng",
    y = "lat",
    linestring_id = "ID_arquivo",
    keep = TRUE,
  ) %>%
  st_set_crs(4326) %>%
  st_write(
    dsn = here(caminho_export_rotas, "rotas.gpkg"),
    layer = "rotas",
    quiet = TRUE,
    append = FALSE
  )
